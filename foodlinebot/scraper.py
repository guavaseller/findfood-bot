...
class GetPlace(Place):
    ...
    def scrape1(self):
        ...
        response = requests.get(url=
            "https://www.google.com.tw/maps/search/" + self.area , headers={ 'user-agent': user_agent.random })
...
class IFoodie(Food):
    ...
    def scrape(self):
        response = requests.get(
            ...
            "&sortby=rating&opening=true"
            )
        
        more[0] = str("https://ifoodie.tw/explore/" + self.area +
            "/list/" + self.category +
            "?priceLevel=" + self.price +
            "&sortby=rating&opening=true")
